...
function PaginationInnerWrapper<T, SearchFuncProps extends SearchParamsProps>({
  children,
  threshold,
  loadingComponent,
}: PaginationWrapperProps<T, SearchFuncProps>) {
  const providerProps = usePaginationProvider();
  const props = providerProps?.props<T, SearchFuncProps>();
 ...
 * @param defaultData - The initial data to display, rendered on the server
...
function PaginationWrapper<T, SearchFuncProps extends SearchParamsProps>(
  props: PaginationWrapperProps<T, SearchFuncProps> &
    Partial<PaginationProviderProps<T, SearchParamsProps>>
) {
  const providerProps = usePaginationProvider();
  if (providerProps)
    return (
